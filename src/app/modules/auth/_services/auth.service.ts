...
export class AuthService implements OnDestroy {
  ...
  login(email: string, password: string): Observable<any> {
    ...
    return this.http
      .post<any>(
        `${environment.apiUrlG12Connect.users}/auth`,
        { email, password, platform: 'conexion12' },
        { headers: header }
      )
      .pipe(
        map((auth: any) => {
  ...
  refreshToken() {
    return this.http
      .post<any>(`${environment.apiUrlG12Connect.users}/auth/refresh-token`, {
        token: this._storageService.getItem('auth').token,
      })
      .pipe(
        map((auth: any) => {
          return auth;
  ...
  forgotPassword(data: { email: string; url: string }): Observable<Response> {
    this.isLoadingSubject.next(true);
    return this.http
      .post<Response>(
        `${environment.apiUrlG12Connect.users}/auth/send-recovery`,
        JSON.stringify(data),
        { headers: header }
      )
      .pipe(
  ...
  changePassword(payload) {
    return this.http
      .post<any>(
        `${environment.apiUrlG12Connect.users}/auth/reset-password`,
        payload,
        { headers: header }
      )
      .pipe(
