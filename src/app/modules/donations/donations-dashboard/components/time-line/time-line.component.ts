...
export class TimeLineComponent implements OnInit {
  ...
  //METODO PARA CONSULTAR LAS DONACIONES DE G12DONATIONS
  getDonations() {
    //CONSULTAMOS EL ENDPOINT
    this._donationsServices.getDonationsMCI().subscribe((res: any) => {
      //AGREGAMOS LA RESPUESTA A LAS DONACIONES
      this.donations = res;
      this.cdr.detectChanges();
    });
  }

  //METODO PARA CONSUILTAR LA DATA DE LA GRAFICA
  getData() {
    this._donationsServices
      .getTimeLineTotals({
        type_filter: this.type_filter.value,
        filter:
          this.type_filter.value == 1
            ? this.selectedDonation.value.id
            : this.payment_method.value,
        currency: this.currency.value,
      })
      .subscribe((res) => {
        //VALIDAMOS SI EXISTEN VALORES EN LA CONSULTA PARA MOSTRAR O NO MOSTRAR LA GRAFICA
        this.showChart = validateChartValues(
          res['series'][0].data.concat(res['series'][1].data)
        );
        if (this.showChart) {
            //CREAMOS LA CHART CON LAS SERIES Y LAS CATEGORIAS
          this.createChartTimeLine(
            res['series'],
            res['categories'],
            this.currency.value
          );
  ...
  createChartTimeLine(series, categories, currency) {
    this.linewithDataChart = chartTimeLine(
      series, //CREAMOS LAS SERIES
      {
        ...
        zoom: {
          //DESHABILITAMOS EL ZOOM
          enabled: false,
        },
        toolbar: {
          //DESHABILITAMOS LAS BARRAS DE HERRAMIENTAS
          show: false,
        },
      },
      ...
      },
      {
        ...
        row: {
          //CREAMOS LOS VALORES DE LAS FILAS
