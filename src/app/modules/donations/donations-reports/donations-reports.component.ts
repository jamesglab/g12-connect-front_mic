...
export class DonationsReportsComponent implements OnInit {
  ...
  getTransactions(event?) {
    ...
    this._donations
      .getTransaccionsDonations({
        init_date: this.range.get('start').value
          ? new Date(moment(this.range.get('start').value).format()).getTime()
          : null,
      ...
      .subscribe((res: any) => {
        ...
        res.transactions.map((item, i) => {
          ...
          res.transactions[i].transaction.payment_method =
            this.validatePaymentMethod(item.transaction.payment_method);
        });
        if (!this.dataSource) {
  ...
  getTransactionsByHours(paginator) {
    ...
    this._donations
      .getTransaccionsDonations({
        init_date: this.rangeHours.get('start').value
          ? new Date(
              moment(this.rangeHours.get('start').value).format()
            ).getTime()
          : null,
        finish_date: this.rangeHours.get('end').value
          ? new Date(
              `${moment(this.rangeHours.get('end').value).format(
                'YYYY-MM-DD'
              )}T23:59:00.000`
            ).getTime()
          : null,
        ...
        if (this.info_to_export.length > 0) {
          ...
          this.info_to_export.map((item) => {
            const newData = {
              ...
              Total:
                item?.transaction.payment_gateway.toString().toUpperCase() ==
                'STRIPE'
