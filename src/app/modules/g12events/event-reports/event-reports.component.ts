    ...
    if (this.info_to_export.length > 0) {
      ...
      this.info_to_export.map(item => {
        const newData = {
          ...
          Pastor: item.pastor?.name ? `${item.pastor.name} ${item.pastor.last_name}` : 'N/A',
          'Lider Doce': item.leader?.name ? `${item.leader.name} ${item.leader.last_name}` : 'N/A',
          'Pastor de Sede': item.pastor_church ? `${item.pastor_church.name} ${item.pastor_church.last_name}` : 'N/A',
